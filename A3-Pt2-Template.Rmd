...
# Abstract
In this report, we are analyzing the relationships between the data results from hurricane intensity, sea ice extent, and climate awareness using the cleaned datasets assigned for completion in part 1. Through the combination of presented/shown insights from these three data sources, we can show and explore some interesting themes, such as whether hurricane strength is actually related to sea ice extent and in what ways.
Further analyzing the 3 datasets and their relations, we can also make connections to determine whether climate awareness correlates with specifec countries and how etc... Through the use and ability to undersand, summary statistics, visualizations, and correlations, we can investigate and conclude with the help of supporting insights, the issues around climate change perceptions/impacts and the relation shown.
