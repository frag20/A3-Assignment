...
at_cyclone <- str_c(cyclone_data_address, at_cyclone_filename, sep = "") |>
    ...
    separate_wider_position(
        BasinNumberYear,
        # Specify the widths
         widths=c(Basin=2,Number=2,NameYear=4),too_few = "align_start"
    ) |>
    separate_wider_position(
        `1`,
        # Specify the widths
        widths=c(ObservYear=4,Month=2,Day=2), too_few="align_start"
    ) |>
    separate_wider_position(
        `2`,
        # Specify the widths
...
```{r error=TRUE}
cyclones_data_update_0 <- rbind(np_cyclone, at_cyclone)
...
cyclones_data_update_2 <- cyclones_data_update_1 |>
    mutate(
       date=make_datetime(year=ObservYear, month=Month,day=Day)
    )
...
### counts as 1 Plots and Summary Statistics!
...
ice_extent_yearly <- ice_extent_daily |>
  ...
  summarise (
    max_extent = max(ice_extent, na.rm = TRUE), 
    min_extent = min(ice_extent, na.rm = TRUE)
  ) |>
  pivot_longer(
    cols = c(max_extent, min_extent), 
    names_to = "name", 
    values_to = "value"
  )
...
file_name <- "ice_extent_yearly.parquet"
write_parquet(ice_extent_yearly, file_name)
...
# No Opinions on climate data from Meta (from A2)
...
covid_2020_match_country <- aware_pop|>
  mutate(
    country = case_when(
      country == "United States" ~ "United States of America",
      country == "South Korea" ~ "Republic of Korea",
      country == "Russia" ~ "Russian Federation",
      # Add any other specific country replacements needed
      TRUE ~ country))
